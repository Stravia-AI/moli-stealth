use moli_core::browser_host::{
    BrowserAuxiliaryNavigationKind, BrowserInitialEmptyDocumentSeed, BrowserPageOwnerKey,
...
pub(super) fn emit_created_target_protocol_events(
    ...
    let creation_fact_projected = match events.browser_fact.as_ref() {
        Some(browser_fact) if conn.target_created_fact_matches_current_target(browser_fact) => {
            tracing::trace!(
                browser_fact_sequence = browser_fact.envelope().sequence().get(),
                target_id = events.target_id,
                "projecting live Target creation from exact Browser fact"
            );
            true
        }
        Some(browser_fact) => {
            tracing::error!(
                browser_fact_sequence = browser_fact.envelope().sequence().get(),
                target_id = events.target_id,
                "suppressing stale Target creation projection after Target instance replacement"
            );
            false
        }
        None => {
            tracing::error!(
                target_id = events.target_id,
                "suppressing Target creation projection without an exact Browser fact"
            );
            false
        }
    };
    if has_discovery && creation_fact_projected {
        let mut event_plan = conn.target_created_event_plan(&events.target_id);
        if project_bidi_lifecycle {
            event_plan = event_plan.without_target_lifecycle_automation_sidecars();
        }
        for event in event_plan {
            out.push_target_background_event(event);
        }
    }
    if creation_fact_projected && project_bidi_lifecycle {
        out.push_target_background_event(BackgroundProtocolEvent::automation_only(
...
pub(super) fn execute_devtools_create_target_command(
    ...
    conn.debug_assert_browser_context_topology_projection();
    if conn.registered_browser_context_count() == 0 {
        let id = conn.gen_bc_id();
        conn.try_insert_browser_context(conn.new_browser_context(id))?;
    }
    ...
    let (
        ...
        let replacing_default_placeholder =
    ...
    let target_projection = if creating_background_target {
        conn.register_background_target_with_creation_metadata_projection(
            &browser_context_id,
            &target_id,
            creation_metadata,
    ...
    } else if replacing_default_placeholder {
        conn.replace_active_target_with_creation_metadata_projection(
            default_target_id,
            &target_id,
            creation_metadata,
            |bc, target_handle, page_residence, session_storage_access| {
                bc.bind_new_active_target_registration(
                    target_handle,
                    page_residence,
                    session_storage_access,
                );
        ...
        conn.register_active_target_with_creation_metadata_projection(
            &target_id,
            creation_metadata,
            |bc, target_handle, page_residence, session_storage_access| {
                if activating_created_target {
                    bc.stage_active_target_demoting_current_with_browser_handles(
                        target_handle,
                        page_residence,
                        session_storage_access,
                        None,
    ...
    let browser_fact = match target_projection {
        Ok(registration) => registration.into_browser_fact(),
        Err(error) => {
            restore_previously_active_browser_context(conn, restore_browser_context_id.as_deref());
    ...
    if command.url != initial_empty_document_url {
        // Chromium gives Target.createTarget(url) one initial auto_toplevel
        // history entry for url. The implementation still materializes an
        // internal about:blank document for target setup, so Browser Core must
        // replace that bookkeeping entry when the requested URL commits.
        conn.mark_next_navigation_history_replace_initial_empty_document_for_owner(
            &BrowserPageOwnerKey::new(browser_context_id, target_id.clone()),
        );
    }
...
fn activate_browser_context_for_create_target(
    ...
    if let Some(wanted_id) = command.browser_context_id.as_ref() {
        ...
        {
            conn.try_insert_browser_context(
                conn.new_browser_context(wanted_id.as_str().to_owned()),
            )?;
        }
        match conn.try_activate_browser_context_by_id(wanted_id.as_str()) {
            Ok(_) => return Ok(()),
            Err(error) if !error.is_unknown_context() => return Err(error.into()),
            Err(_) => {}
        }
    ...
    match conn.try_activate_browser_context_by_id(browser_context_id) {
        Ok(_) => Ok(()),
        Err(error) if !error.is_unknown_context() => Err(error.into()),
        Err(_) => Err(DevToolsError::new(
            DevToolsErrorKind::NoSuchTarget,
            "NoSuchTarget",
        )),
    }
...
pub(crate) async fn create_popup_target_from_renderer_output_background_events_async(
    ...
    let mut initial_document_seed = BrowserInitialEmptyDocumentSeed::new("about:blank");
    if let Some(creator) = popup_creator.clone() {
        initial_document_seed = initial_document_seed.with_creator(creator);
    }
    if let Some(storage_key) = initial_empty_document_storage_key.clone() {
        initial_document_seed = initial_document_seed.with_storage_key(storage_key);
    }
    let mut creation_metadata =
        BrowserTargetCreationMetadata::with_initial_empty_document(initial_document_seed);
    if let Some(session_storage_store) = session_storage_store.as_ref() {
        creation_metadata =
            creation_metadata.with_session_storage_store(session_storage_store.clone());
    }

    let registration = conn.register_background_target_with_creation_metadata_projection(
        &browser_context_id,
        &target_id,
        creation_metadata,
    ...
    let browser_fact = match registration {
        Ok(registration) => registration.into_browser_fact(),
        Err(error) => {
            tracing::warn!(
                browser_context_id,
                target_id,
                error = %error,
                "popup Target registration projection was rejected"
            );
            return None;
        }
    };
    ...
    let creation_fact_projected = match browser_fact.as_ref() {
        Some(browser_fact) if conn.target_created_fact_matches_current_target(browser_fact) => {
            tracing::trace!(
                browser_fact_sequence = browser_fact.envelope().sequence().get(),
                target_id,
                "projecting popup Target creation from exact Browser fact"
            );
            true
        }
        Some(browser_fact) => {
            tracing::error!(
                browser_fact_sequence = browser_fact.envelope().sequence().get(),
                target_id,
                "suppressing stale popup Target creation projection"
            );
            false
        }
        None => {
            tracing::error!(
                target_id,
                "suppressing popup Target creation projection without an exact Browser fact"
            );
            false
        }
    };
    if creation_fact_projected {
        let project_bidi_lifecycle = conn.webdriver_bidi_target_lifecycle_projection_enabled();
        if conn.has_any_target_discovery() {
            if project_bidi_lifecycle {
                for event in conn
                    .target_created_event_plan(&target_id)
        ...
        // of whether that frontend subscribed to `Target.targetCreated`.
        // Preserve that separation while the BiDi projector is installed:
        // CDP discovery controls only the CDP notification, while the accepted
        // auxiliary browsing-context action publishes one typed lifecycle
        // projection. This is especially important for popup creation that
        // settles after the causing Runtime command response.
        if project_bidi_lifecycle {
    ...
    if let Some(activation) = activation {
        ...
        match conn.publish_renderer_auxiliary_navigation(
            ...
            Ok(false) => tracing::debug!(
                browser_context_id,
                target_id,
                "dropping initial auxiliary navigation after its Page retired"
            ),
            Err(error) => tracing::error!(
                %error,
                browser_context_id,
                target_id,
                "initial auxiliary navigation could not enter Browser Host"
            ),
        }
    }
    Some(target_id)
...
pub(super) async fn ensure_popup_initial_document_page_async(
    conn: &mut CdpConnection,
    target_id: &str,
) -> bool {
...
pub(super) fn publish_target_url_navigation_if_allowed(
    conn: &mut CdpConnection,
    target_id: &str,
) -> Result<bool, crate::conn::BrowserOwnerInputPublicationError> {
    ...
    };
    {
        ...
        route_scope
            .conn_mut()
            .publish_initial_target_navigation_for_session_owner(None)
    }
}
...
async fn close_target_inner_async(
    ...
    if conn.browser_context.as_ref().is_some_and(|bc| {
        ...
        let session_id = conn
            .browser_context
            .as_ref()
            .and_then(|browser_context| browser_context.primary_session_id_for_target(&target_id))
            .map(str::to_owned);
        ...
        let renderer_output_predecessor =
            ...
            .await;
        settle_target_close_after_pending_fetches(
            conn,
            command_context,
...
/// Admits `Target.closeTarget` after preserving any final renderer
/// publication produced by pending-fetch cancellation.
///
/// The frontend result and the Browser Host action are independent outputs of
/// this command turn. A paused request can first produce a terminal renderer
/// record; the command completion fence projects that exact record before the
/// application loop can select the independently queued Browser Host turn.
fn settle_target_close_after_pending_fetches(
    conn: &mut CdpConnection,
    command_context: &mut crate::conn::CommandDispatchContext,
    renderer_output_predecessor: Option<moli_core::RendererOutputFence>,
    termination: moli_core::browser_host::BrowserTargetTerminationRequest,
) -> Result<(), DevToolsError> {
    if let Some(predecessor) = renderer_output_predecessor {
        command_context.set_renderer_output_predecessor(predecessor);
    }

    let input = moli_core::browser_host::BrowserOwnerInput::target_termination(termination);
    conn.publish_browser_owner_input(input).map_err(|error| {
        DevToolsError::new(
            DevToolsErrorKind::Internal,
            format!("BrowserHostTargetCloseAdmissionFailed: {error}"),
        )
    })
}
...
pub(super) fn execute_devtools_get_target_info_command(
    ...
    let target_infos = super::browser_context::current_devtools_target_infos(conn)?;
    ...
    if let Some(page_target_id) = conn.page_target_id_for_tab_target_id(wanted)
        && target_infos.iter().any(|target_info| {
            target_info
                .target_id
                .as_ref()
                .is_some_and(|target_id| target_id.as_str() == page_target_id)
        })
        && let Some(target_info) = conn.tab_target_info(wanted)
    {
    ...
    let target_info = target_infos
        ...
        .find(|target_info| {
            target_info
                .target_id
                .as_ref()
                .is_some_and(|target_id| target_id.as_str() == wanted)
        })
        .ok_or_else(|| DevToolsError::new(DevToolsErrorKind::NoSuchTarget, "UnknownTargetId"))?;
    if let Some(message) = super::transient_no_page_devtools_target_info_error(conn, &target_info) {
        return Err(DevToolsError::new(DevToolsErrorKind::Internal, message));
...
pub(super) async fn complete_set_auto_attach_command_async(
    ...
    let mut side_effects = events::TargetProtocolSideEffects::default();
    let restore_browser_context = conn
        ...
        .and_then(|snapshot| {
            snapshot
                .contexts()
                .iter()
                .find(|context| context.is_selected())
                .cloned()
        });
    let result = set_auto_attach_inner_async(
        conn,
        &mut side_effects,
        auto_attach,
        owner_session_id,
    ...
    .await;
    if let Some(restore_browser_context) = restore_browser_context.as_ref()
        && conn.browser_context_target_snapshot_is_current(restore_browser_context)
    {
        restore_previously_active_browser_context(
            conn,
            Some(restore_browser_context.browser_context_id()),
        );
    }
...
async fn set_auto_attach_inner_async(
    ...
    let browser_snapshot = conn.capture_browser_top_level_target_snapshot()?;
    for context_snapshot in browser_snapshot.contexts() {
        if !conn.browser_context_target_snapshot_is_current(context_snapshot) {
            continue;
        }
        let context_id = context_snapshot.browser_context_id().to_owned();
        if !conn.activate_browser_context_by_id_async(&context_id).await
            || !conn.browser_context_target_snapshot_is_current(context_snapshot)
        {
            continue;
        }
        if auto_attach {
            ...
            let pending_attach_targets = {
                ...
                if !attach_page_targets {
                    ...
                    context_snapshot
                        .targets()
                        .iter()
                        .filter(|snapshot| conn.browser_target_state_snapshot_is_current(snapshot))
                        .filter(|snapshot| {
                            let target_has_primary_session =
                                if bc.active_target_id() == Some(snapshot.target_id()) {
                                    bc.has_active_session()
                                } else {
                                    bc.primary_session_id_for_target(snapshot.target_id())
                                        .is_some()
                                };
            ...
            let pending_attach_tab_targets = {
                if !attach_tab_targets {
                    ...
                    context_snapshot
                        ...
                        .filter_map(|snapshot| {
                            let tab_target_id =
                                conn.tab_target_id_for_page_target_id(snapshot.target_id())?;
                            should_auto_attach_tab_target_for_owner(
                                conn,
                                owner_session_id,
                                tab_target_id,
                            )
                            .then(|| (snapshot.clone(), tab_target_id.to_owned()))
                        })
                        .collect()
                }
            };
            ...
            let attached_targets = pending_attach_targets
                .into_iter()
                .map(|target_snapshot| (target_snapshot, conn.gen_session_id()))
                .collect::<Vec<_>>();
            let attached_tab_targets = pending_attach_tab_targets
                .into_iter()
                .map(|(target_snapshot, tab_target_id)| {
                    (target_snapshot, tab_target_id, conn.gen_session_id())
                })
                .collect::<Vec<_>>();
            ...
            let promote_target = {
                ...
                if !attach_page_targets || bc.has_loaded_page() {
                    ...
                    let target_id = bc
                        ...
                        .iter()
                        .rposition(|target| {
                            bc.primary_session_id_for_target(target.target_id())
                                .is_none()
                    ...
                    target_id.and_then(|target_id| context_snapshot.target(&target_id).cloned())
                }
            };
            let mut exact_attached_targets = Vec::with_capacity(attached_targets.len());
            for (target_snapshot, session_id) in attached_targets {
                if conn.browser_target_state_snapshot_is_current(&target_snapshot)
                    && conn.prepare_auto_attached_page_session_binding(
                        target_snapshot.target_id(),
                        session_id.clone(),
                    )
                {
                    exact_attached_targets.push((target_snapshot, session_id));
                }
            }
            let attached_targets = exact_attached_targets;
            {
            ...
            let mut exact_attached_tab_targets = Vec::with_capacity(attached_tab_targets.len());
            for (target_snapshot, tab_target_id, session_id) in attached_tab_targets {
                if conn.browser_target_state_snapshot_is_current(&target_snapshot)
                ...
                {
                    exact_attached_tab_targets.push((target_snapshot, tab_target_id, session_id));
                }
            }
            let attached_tab_targets = exact_attached_tab_targets;
            if let Some(promote_target) = promote_target
                && conn.browser_target_state_snapshot_is_current(&promote_target)
            {
                let promote_target_id = promote_target.target_id().to_owned();
                match conn
                    ...
                    .await
                {
                    ...
                    Err(error) => {
                        tracing::warn!(
                            ...
                            error = %error,
                            "same-context Target promotion was rejected during auto-attach"
                        );
                    }
                }
            }
            let attached_target_ids = attached_targets
                .iter()
                .map(|(target_snapshot, session_id)| {
                    (target_snapshot.target_id().to_owned(), session_id.clone())
                })
                .collect::<Vec<_>>();
            ensure_initial_document_for_attached_page_targets_async(conn, &attached_target_ids)
                .await;
            for (target_snapshot, session_id) in &attached_targets {
                if !conn.browser_target_state_snapshot_is_current(target_snapshot) {
                    continue;
            ...
            for (target_snapshot, target_id, session_id) in attached_tab_targets {
                ...
                let Some(ti) = conn.tab_target_info(&target_id) else {
                    conn.rollback_prepared_attach_session_without_event_async(&prepared_session)
                        .await;
                    continue;
                };
            ...
            for (target_snapshot, session_id) in attached_targets {
                ...
                let target_id = target_snapshot.target_id().to_owned();
                let ti = match conn.project_top_level_target_snapshot(&target_snapshot) {
                    Ok(target_info) => target_info,
                    Err(error) => {
                        warn_target_protocol_side_effect_failure(
                            &target_id,
                            "project_attached_to_target",
                            &error.message,
                        );
...
async fn auto_attach_child_page_for_tab_session_async(
    ...
    let target_snapshot = match conn.capture_browser_top_level_target_snapshot() {
        Ok(snapshot) => snapshot.target(&page_target_id).cloned(),
        Err(error) => {
            warn_target_protocol_side_effect_failure(
                &page_target_id,
                "capture_tab_child_auto_attach_snapshot",
                &error.message,
            );
            return;
        }
    ...
    if !conn
        .activate_browser_context_by_id_async(target_snapshot.browser_context_id())
        .await
        || !conn.browser_target_state_snapshot_is_current(&target_snapshot)
    {
        return;
    }
    let target_has_primary_session = {
        ...
        if bc.active_target_id() == Some(page_target_id.as_str()) {
            ...
            bc.primary_session_id_for_target(&page_target_id).is_some()
        }
    };
    ...
    let target_info = match conn.project_top_level_target_snapshot(&target_snapshot) {
        ...
        Err(error) => {
            warn_target_protocol_side_effect_failure(
                &page_target_id,
                "project_tab_child_auto_attach_snapshot",
                &error.message,
            );
            conn.rollback_prepared_attach_session_without_event_async(&prepared_session)
                .await;
...
mod protocol_neutral_tests {
    ...
    use moli_core::{
        ...
        browser_host::BrowserHostActor,
    };
    use serde_json::Value;

    use crate::conn::{
        BrowserContext, BrowserTargetTerminationProjectionKind, CdpConnection, Cmd,
    ...
    use super::{
        ...
        build_cdp_create_target_command, settle_target_close_after_pending_fetches,
    };

    #[test]
    fn target_close_with_renderer_predecessor_rejects_without_browser_host() {
        ...
        let termination = conn
            ...
            .expect("live Target should expose exact termination authority");
        let stream =
            RendererOutputStreamIdentity::new_page_for_protocol_test(PageId::new_for_testing(61));
        let predecessor =
            RendererOutputFence::new_for_test(RendererOutputCursor::new_for_test(stream, 7));
        let predecessor_cursor = predecessor.cursor();
        let mut command_context = CommandDispatchContext::default();

        let error = settle_target_close_after_pending_fetches(
            &mut conn,
            &mut command_context,
            Some(predecessor),
            termination,
        )
        .expect_err("Target.closeTarget requires a live Browser Host");

        assert_eq!(
            error.kind,
            crate::devtools_runtime::DevToolsErrorKind::Internal
        );
        assert!(
            error
                .message
        ...
        assert_eq!(
            command_context
                ...
                .map(|predecessor| predecessor.cursor()),
            Some(predecessor_cursor),
            "Host rejection must not erase the terminal renderer fence"
        );
        ...
        assert_eq!(
            ...
            "rejected Target.closeTarget admission must leave the Target live"
        );
    }

    #[test]
    fn target_close_with_renderer_predecessor_publishes_directly_to_browser_host() {
        let mut conn = CdpConnection::default();
        conn.install_default_browser_target();
        let target_id = conn
            .browser_context
        ...
        let termination = conn
            ...
            .expect("live Target should expose exact termination authority");
        let (browser_host, handle) = BrowserHostActor::new(conn.browser_host_state());
        conn.install_browser_host_handle(handle);
        let stream =
            RendererOutputStreamIdentity::new_page_for_protocol_test(PageId::new_for_testing(62));
        let predecessor =
            RendererOutputFence::new_for_test(RendererOutputCursor::new_for_test(stream, 8));
        let predecessor_cursor = predecessor.cursor();
        let mut command_context = CommandDispatchContext::default();
        ...
        assert_eq!(
            conn.browser_context
                ...
                .and_then(BrowserContext::active_target_id),
            Some(target_id.as_str()),
            "publishing the Host input cannot execute Target.closeTarget"
        );
    }
