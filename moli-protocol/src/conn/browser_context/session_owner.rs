use super::*;
use crate::conn::{
    ServiceWorkerTargetState, SharedWorkerTargetState, TargetEventPlan, TargetHostDelta,
};
...
impl CdpConnection {
    ...
    /// the dedicated Target metadata projector instead.
    pub(crate) fn frontend_attachment_info_changed_event_plan(
        &mut self,
        target_id: &str,
    ) -> TargetEventPlan {
...
impl CdpConnection {
    ...
    fn commit_prepared_attach_event_plan_with_attached_state_delta(
        ...
        let attached_state_delta_plan = should_emit_attached_state_delta
            .then(|| self.frontend_attachment_info_changed_event_plan(&target_id));
    ...
    /// Detaches a SharedWorker/ServiceWorker session whose exact worker
    /// target was already removed by BrowserContext disposal.
    ///
    /// These routes have no renderer-side primary-Page cleanup. Resolving the
    /// physical Context from the captured route avoids selecting an inactive
    /// Context merely to clear connection-local attachment state.
    pub(crate) fn detach_worker_session_with_binding_cleanup_event_plan(
        &mut self,
        cleanup_plan: crate::conn::TargetSessionDetachCleanupPlan,
    ) -> crate::conn::target::TargetEventPlan {
        let session_id = cleanup_plan.session_id().to_owned();
        ...
        if let Some(route) = route.as_ref()
            && let Some(browser_context_id) = route.browser_context_id()
            && let Some(browser_context) = self.browser_context_by_id_mut(browser_context_id)
        {
            match route {
                crate::conn::CdpSessionRoute::SharedWorkerTarget { .. } => {
                    let _ = browser_context.detach_shared_worker_target_session(&session_id);
                }
                crate::conn::CdpSessionRoute::ServiceWorkerTarget { .. } => {
                    let _ = browser_context.detach_service_worker_target_session(&session_id);
                }
                _ => {
                    tracing::warn!(
                        session_id,
                        ?route,
                        "worker disposal received a non-worker attachment route"
                    );
                }
            }
        }
...
fn browser_context_session_route(
    ...
    browser_context
        ...
        .filter(|target_id| !browser_context.is_active_target(target_id))
        .map(|target_id| CdpSessionRoute::BackgroundTarget {
            browser_context_id: browser_context.id.clone(),
            target_id: target_id.to_owned(),
        })
        .or_else(|| {
