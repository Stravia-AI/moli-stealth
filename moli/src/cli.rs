...
mod tests {
    ...
    fn client_certificate_uses_curl_style_password_and_colon_escaping() {
        ...
        let windows = ClientCertificate::from_str(r"C:\certs\client.p12")
            .expect("Windows client certificate path should parse");
        assert_eq!(windows.path(), std::path::Path::new(r"C:\certs\client.p12"));
        assert_eq!(windows.password(), None);

        let windows_with_password = ClientCertificate::from_str("C:/certs/client.p12:secret")
            .expect("Windows client certificate path with password should parse");
        assert_eq!(
            windows_with_password.path(),
            std::path::Path::new("C:/certs/client.p12")
        );
        assert_eq!(windows_with_password.password(), Some("secret"));
    }
