...
use crate::{
    LayoutCssImageReference, LayoutElementSemantics, LayoutError, LayoutGridGeometry, LayoutPoint,
    LayoutPseudo, LayoutScrollbarAxis, LayoutScrollbarColors, LayoutScrollbarGutter,
    ...
    N: Copy + Debug + Eq + Hash,
{
    ...
    pub(crate) fn document_scrolling_element(&self) -> Option<N> {
        ...
        self.source_mapping
            .get(&body)
            .is_some_and(|id| self.is_viewport_defining_body(*id))
            .then_some(body)
    }
    ...
    /// Whether this box owns a scrollport after root/body overflow propagation.
    pub(crate) fn establishes_scroll_container(&self, id: LayoutBoxId) -> bool {
        if id == self.root {
            self.viewport_scroll_policy.establishes_scroll_container()
        } else if self.is_viewport_defining_body(id) {
            false
        } else {
