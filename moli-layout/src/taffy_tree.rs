...
use taffy::{
    ...
    FontBaseline, IntrinsicSizeResult, Layout, LayoutBlockContainer, LayoutFlexboxContainer,
    LayoutGridContainer, LayoutInput, LayoutOutput, LayoutPartialTree, Line, LogicalBoxStrut,
    LogicalOffset, LogicalSize, LogicalStaticPosition, MaybeMath, MaybeResolve, NodeId,
    ...
    N: Copy + Debug + Eq + Hash,
{
    ...
    world.viewport_layout.style = Style {
        ...
        // scrollbar gutters as its non-painted border so in-flow root content
        // and fixed descendants share one reduced containing rectangle.
        border: taffy::Rect {
    ...
    N: Copy + Debug + Eq + Hash,
{
    ...
    fn get_scrollbar_insets(&self, node_id: NodeId) -> taffy::Rect<f32> {
        if self.is_viewport_taffy_node(node_id) {
            ...
            // containing block. Do not reserve the same space again.
            return taffy::Rect::ZERO;
        }
        let id = LayoutBoxId::from_taffy(node_id);
        if id == self.root || self.is_viewport_defining_body(id) {
            // Root/body overflow has already propagated to the viewport.
            taffy::Rect::ZERO
        } else {
    ...
    N: Copy + Debug + Eq + Hash,
{
    ...
    fn compute_inline_formatting_context(
            ...
            (measurement, inline_context.as_mut())
        {
            ...
            let logical_scrollbar_insets = writing_direction.to_logical_box_strut(scrollbar_insets);
            let natural_first_baseline = measurement
                ...
                .map(|line| {
                    logical_border.block_start
                        + logical_scrollbar_insets.block_start
            ...
            let content_offset = Point {
                x: border.left + scrollbar_insets.left + padding.left,
                y: border.top + scrollbar_insets.top + padding.top,
            };
        ...
        if inputs.run_mode == RunMode::PerformLayout {
            self.layout_custom_context_out_of_flow_children(
                id,
                &mut output,
                padding,
                border,
