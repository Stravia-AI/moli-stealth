...
use rows::{
    TableRowConstraint, TableSectionConstraint, apply_fixed_section_block_sizes,
    distribute_table_block_size_to_sections, normalize_row_percentages,
};
...
impl TableGroupedChildren {
    ...
    fn sections(&self) -> impl Iterator<Item = (LayoutBoxId, bool)> + '_ {
        self.header
            .iter()
            .copied()
            .map(|section| (section, false))
            .chain(self.bodies.iter().copied().map(|section| (section, true)))
            .chain(self.footer.iter().copied().map(|section| (section, false)))
    }
}
...
struct TableContext {
    ...
    cells: Vec<TableCell>,
    rows: Vec<TableRow>,
    row_constraints: Vec<TableRowConstraint>,
    sections: Vec<TableSectionConstraint>,
    ...
    N: Copy + Debug + Eq + Hash,
{
    ...
    let natural_grid_block_size = block_size_is_requested.then(|| {
        ...
        let section_growth = context.capture_natural_row_constraints();
        context
            .writing_mode
    ...
    if block_size_is_requested
        && context.distribute_rows_to_table_block_size(grid_inputs, output.size)
    {
        let mut wrapper = TableTreeWrapper {
            world,
            context: &mut context,
    ...
    N: Copy + Debug + Eq + Hash,
{
    ...
    for (section, is_body) in grouped_children.sections() {
        ...
        if row_count > 0 {
            let kind = world.boxes[section.index()].kind;
            let (specified_block_size, percent, is_constrained) = if matches!(
                kind,
                LayoutBoxKind::TableRowGroup
                    | LayoutBoxKind::TableHeaderGroup
                    | LayoutBoxKind::TableFooterGroup
                ...
                specified_table_block_constraint(
                    &world.boxes[section.index()].style.taffy,
                    writing_mode,
                )
            } else {
                (None, None, false)
            };
            sections.push(TableSectionConstraint {
                start_row,
                row_count,
    ...
    for cell in &cells {
        if cell.row_span == 1 {
            let (_, percent, is_constrained) =
                specified_table_block_constraint(&cell.style, writing_mode);
            row_constraints[cell.row].encompass_cell(percent, is_constrained);
        }
    }
    for section in &sections {
        let end_row = section
            ...
            .min(row_constraints.len());
        normalize_row_percentages(&mut row_constraints[section.start_row..end_row]);
    }
...
impl TableContext {
    ...
    fn resolve_used_table_box(&mut self, inputs: LayoutInput) {
        ...
        let constraint_space = inputs.constraint_space(self.writing_mode);
        ...
        if self.style.box_sizing == taffy::BoxSizing::ContentBox {
            // Grid has no table-specific outer border-spacing inset, so the
            // adapter represents it as padding. That inset still belongs
            // inside the authored CSS table content box: letting Grid add it
            // outside a content-box size would enlarge the table twice.
            // Materialize only dimensions resolvable in this constraint
            // space; auto/intrinsic and cyclic percentages remain authored.
            let physical_percentage_basis = self
                .writing_mode
                .to_physical(constraint_space.percentage_resolution_size);
            let synthetic_padding = Size {
                width: spacing_padding.left + spacing_padding.right,
                height: spacing_padding.top + spacing_padding.bottom,
            };
            self.style.size = absorb_synthetic_table_padding(
                self.style.size,
                physical_percentage_basis,
                synthetic_padding,
            );
            self.style.min_size = absorb_synthetic_table_padding(
                self.style.min_size,
                physical_percentage_basis,
                synthetic_padding,
            );
            self.style.max_size = absorb_synthetic_table_padding(
                self.style.max_size,
                physical_percentage_basis,
                synthetic_padding,
            );
    ...
    /// Freeze ROWMIN from the initial indefinite pass before the table's own
    /// block-size can distribute excess space.
    fn capture_natural_row_constraints(&mut self) -> f32 {
        ...
        for (row, size) in self.row_constraints.iter_mut().zip(row_sizes) {
            row.block_size = size.max(0.0);
        }

        let spacing = self.outer_border_spacing.height.max(0.0);
        let section_growth =
            apply_fixed_section_block_sizes(spacing, &mut self.sections, &mut self.row_constraints);
        self.materialize_used_row_tracks();
        section_growth
    }

    /// Apply CSS Tables' section/row distribution to the table block-size
    /// selected by the generic outer box algorithm.
    fn distribute_rows_to_table_block_size(
        &mut self,
        inputs: LayoutInput,
        provisional_outer_size: Size<f32>,
    ) -> bool {
        if self.row_constraints.is_empty() || self.sections.is_empty() {
            return false;
        }
        ...
        let outer_block_size = self
            .writing_mode
            .to_logical(provisional_outer_size)
            .block_size;
        let content_block_size =
            (outer_block_size - insets.block_start - insets.block_end).max(0.0);
        let spacing = self.outer_border_spacing.height.max(0.0);
        // Outer border spacing is represented as Grid padding. Blink's table
        // distributor receives it as part of the table-grid block-size and
        // subtracts one spacing before/after all sections.
        let table_grid_block_size = content_block_size + 2.0 * spacing;
        let before = self
            .row_constraints
            .iter()
        ...
        distribute_table_block_size_to_sections(
            ...
            &mut self.sections,
            &mut self.row_constraints,
        );
        let changed = before
            .iter()
            .zip(&self.row_constraints)
            .any(|(before, row)| (before - row.block_size).abs() > f32::EPSILON);
        if !changed {
            return false;
        }
        self.materialize_used_row_tracks();
    ...
    fn materialize_used_row_tracks(&mut self) {
        self.style.grid_template_rows = fixed_grid_tracks(
...
fn collect_rows<N>(
    ...
    row_constraints: &mut Vec<TableRowConstraint>,
    ...
    N: Copy + Debug + Eq + Hash,
{
    match world.boxes[current.index()].kind {
        ...
        LayoutBoxKind::TableRow | LayoutBoxKind::AnonymousTableRow => {
            ...
            let (_, percent, is_constrained) = specified_table_block_constraint(
                &world.boxes[current.index()].style.taffy,
                writing_mode,
            );
            rows.push(TableRow {
            ...
            row_constraints.push(TableRowConstraint {
                block_size: 0.0,
                percent,
                is_constrained,
            });
...
fn place_table_cells(cells: &mut [TableCell], rows: &[TableRow], max_columns: &mut usize) {
    ...
    for row in rows {
        if active_section != Some(row.section) {
            occupied_until.clear();
            active_section = Some(row.section);
        }
...
/// Project only fixed row lengths into ROWMIN's numeric Grid pass.
///
/// A percentage row is a CSS Tables constraint resolved against the final
/// section block-size. Encoding it as a Grid percentage track would resolve
/// it prematurely and then feed that expanded value back as ROWMIN.
fn table_row_minimum_track(dimension: Dimension) -> taffy::TrackSizingFunction {
    if dimension.tag() == taffy::CompactLength::LENGTH_TAG {
...
fn specified_table_block_constraint(
    style: &Style<Atom>,
    writing_mode: taffy::WritingMode,
) -> (Option<f32>, Option<f32>, bool) {
    let block_size = writing_mode.to_logical(style.size).block_size;
    match block_size.tag() {
        taffy::CompactLength::LENGTH_TAG => (Some(block_size.value().max(0.0)), None, true),
        taffy::CompactLength::PERCENT_TAG => (None, Some(block_size.value().max(0.0)), true),
        _ => (None, None, false),
    }
}
...
fn absorb_synthetic_table_padding(
    dimensions: Size<Dimension>,
    percentage_basis: Size<Option<f32>>,
    synthetic_padding: Size<f32>,
) -> Size<Dimension> {
    let absorb = |dimension: Dimension, basis: Option<f32>, padding: f32| {
        dimension
            .maybe_resolve(basis, resolve_stylo_calc_value)
            .map(|value| style_helpers::length((value - padding).max(0.0)))
            .unwrap_or(dimension)
    };
    Size {
        width: absorb(
            ...
            synthetic_padding.width,
        ),
        height: absorb(
            dimensions.height,
            percentage_basis.height,
            synthetic_padding.height,
        ),
    }
}
...
fn normalize_table_cell_block_sizing(style: &mut Style<Atom>, writing_mode: taffy::WritingMode) {
    ...
    let mut min_size = writing_mode.to_logical(style.min_size);
    let mut max_size = writing_mode.to_logical(style.max_size);
    // Fixed cell block sizes contribute to ROWMIN. Percentages instead become
    // row constraints and remain cyclic until the section's final block-size
    // is known; passing them through as Grid item minimums would resolve the
    // same authored percentage twice.
    min_size.block_size = if size.block_size.tag() == taffy::CompactLength::LENGTH_TAG {
        size.block_size
    } else {
        Dimension::auto()
    };
...
fn collapsed_row_leading_spacings(rows: &[TableRow], spacing: f32) -> Vec<f32> {
    let spacing = spacing.max(0.0);
    let mut active_section = None;
    let mut has_visible_row_in_section = false;
    rows.iter()
        .map(|row| {
            let starts_section = active_section != Some(row.section);
            let leading = if starts_section {
                let leading = if active_section.is_some() {
                    spacing
                } else {
                    0.0
                };
                active_section = Some(row.section);
                has_visible_row_in_section = false;
                leading
            } else if !row.collapsed && has_visible_row_in_section {
                spacing
            } else {
                0.0
            };
            has_visible_row_in_section |= !row.collapsed;
            leading
        })
        .collect()
}
