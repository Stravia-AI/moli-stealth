...
use anyhow::Result;
use boring2::{pkcs12::Pkcs12, pkey::PKey, x509::X509};
...
impl TlsCredentials {
    fn new() -> Result<Self> {
        ...
        let key = PKey::private_key_from_der(&client_key.serialize_der())?;
        let certificate = X509::from_der(client_cert.der())?;
        let pkcs12 = Pkcs12::builder()
            .build("test-password", "moli-client", &key, &certificate)?
            .to_der()?;
        fs::write(credentials.dir.join("client.p12"), pkcs12)?;
        Ok(credentials)
    }
    ...
    fn pkcs12_fetch_config(&self) -> FetchConfig {
        let mut config = FetchConfig::default();
        config.set_http_proxy(Some(String::new()));
        config.set_request_timeout_ms(5_000);
        config.set_tls_credentials(
            Some(self.dir.join("ca.pem")),
            Some(self.dir.join("client.p12")),
            None,
            Some("test-password".to_owned()),
        );
        config
...
#[tokio::test]
async fn tls_pkcs12_client_certificate_authenticates() -> Result<()> {
    let credentials = TlsCredentials::new()?;
    let server = TlsServer::spawn(&credentials).await?;
    let client = FetchClient::new(
        &credentials.pkcs12_fetch_config(),
