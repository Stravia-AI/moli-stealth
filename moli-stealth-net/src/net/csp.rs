...
//! `crates/crate::js_runtime::extensions::fetch_ext`.
//!
...
pub enum Source {
    ...
    /// Walmart's CSP includes `'strict-dynamic'` so the challenge vendor's
    /// parser-injected `/akam/13/...` script is blocked even though same-origin.
    StrictDynamic,
...
// twist that makes Walmart's CSP block parser-inserted challenge-vendor scripts.
// ---------------------------------------------------------------------
...
mod tests {
    ...
    /// **The load-bearing case**: Walmart's CSP. The challenge vendor's
    /// parser-injected `/akam/13/3e35295b` script is same-origin but has no
    /// matching nonce — `'strict-dynamic'` causes Chrome to ignore the host
