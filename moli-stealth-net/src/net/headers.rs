...
fn chrome_headers_impl(
    ...
    if include_high_entropy {
        headers.push(("downlink".to_string(), "1.7".to_string()));
        headers.push(("rtt".to_string(), "150".to_string()));
        headers.push((
    ...
    // 9. accept-encoding (Chrome 124+ includes zstd)
    headers.push((
        "accept-encoding".to_string(),
        "gzip, deflate, br, zstd".to_string(),
    ));

    // 10. priority (Chrome 130+)
...
/// Chrome uses the Windows profile value verbatim and zero-pads macOS.
...
fn build_sec_ch_ua_full_version_list(profile: &StealthProfile) -> String {
    ...
    format!("\"Chromium\";v=\"{v}\", \" Not A;Brand\";v=\"99.0.0.0\", \"Google Chrome\";v=\"{v}\"")
}
...
mod tests {
    ...
    #[test]
    fn platform_version_matches_current_chrome_values() {
        assert_eq!(chrome_platform_version("Windows", "10.0"), "10.0");
        assert_eq!(chrome_platform_version("Windows", "11"), "11");
