...
// This is the submodule revision shipped by btls de7ab84fdb58641a2bdfdf9d8ebd7db1dcf4b29b.
// Keep source acquisition deterministic when this patched crate is used outside that git checkout.
const BORINGSSL_GIT_URL: &str = "https://github.com/google/boringssl.git";
const BORINGSSL_GIT_REV: &str = "f1f2556a5dfa59e147d9d47279cc3f7f8a18b433";

// Every OPENSSL_EXPORT introduced by the btls patch set. BoringSSL's pregenerated
// public prefix list predates those patches, so native compilation must extend it.
const PATCHED_PUBLIC_SYMBOLS: &[&str] = &[
    "EVP_aead_aes_256_cbc_sha256_tls",
    "EVP_aead_aes_256_cbc_sha384_tls",
    "SSL_CREDENTIAL_new_raw_public_key_empty",
    "SSL_CREDENTIAL_set1_spki",
    "SSL_CTX_get0_accepted_peer_cert_types",
    "SSL_CTX_set_aes_hw_override",
    "SSL_CTX_set_delegated_credentials",
    "SSL_CTX_set_extension_order",
    "SSL_CTX_set_preserve_tls13_cipher_list",
    "SSL_CTX_set_record_size_limit",
    "SSL_CTX_use_second_keyshare",
    "SSL_get0_accepted_peer_cert_types",
    "SSL_get0_peer_pubkey",
    "SSL_set_aes_hw_override",
    "SSL_set_preserve_tls13_cipher_list",
    "SSL_set_record_size_limit",
    "SSL_use_second_keyshare",
];
...
fn get_boringssl_source_path(config: &Config) -> &Path {
    ...
    SOURCE_PATH.get_or_init(|| {
        ...
        if !submodule_path.join("CMakeLists.txt").exists() {
            println!("cargo:warning=fetching pinned BoringSSL source");
            let _ = fs::remove_dir_all(&src_path);

            run_command(
                Command::new("git")
                    .args(["clone", "--no-checkout", BORINGSSL_GIT_URL])
                    .arg(&src_path),
            )
            .expect("cloning BoringSSL source");
            run_command(
                Command::new("git")
                    .arg("-C")
                    .arg(&src_path)
                    .args(["checkout", "--detach", BORINGSSL_GIT_REV]),
            )
            .expect("checking out pinned BoringSSL revision");
            return src_path;
        }
...
fn prefix_patched_public_symbols(config: &Config) -> io::Result<()> {
    if !config.features.prefix_symbols {
        return Ok(());
    }

    let include_path = get_boringssl_source_path(config)
        .join("include")
        .join("openssl");
    let header_path = include_path.join("prefix_symbols.h");
    let mut header = fs::read_to_string(&header_path)?;
    let base_header = fs::read_to_string(include_path.join("base.h"))?;
    // C entry-point prefixes do not isolate constructors, destructors, or
    // template instantiations of global public structs. Prefix every opaque
    // tag from the pinned public type registry, not just the first link clash.
    let mut symbols: Vec<_> = base_header
        .lines()
        .filter_map(|line| line.strip_prefix("typedef struct "))
        .filter_map(|declaration| declaration.split_ascii_whitespace().next())
        .collect();
    if symbols.is_empty() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "BoringSSL public opaque type declarations not found",
        ));
    }
    symbols.extend_from_slice(PATCHED_PUBLIC_SYMBOLS);
    symbols.sort_unstable();
    symbols.dedup();
    let mut definitions = String::from(
        "// btls native namespace: patched exports and public C++ type identities.\n",
    );
    for symbol in symbols {
        let definition = format!("#define {symbol} BORINGSSL_ADD_PREFIX({symbol})");
        if !header.contains(&definition) {
            definitions.push_str(&definition);
            definitions.push('\n');
        }
    }

    if definitions.lines().count() == 1 {
        return Ok(());
    }

    const INSERT_BEFORE: &str =
        "#if defined(__PRAGMA_REDEFINE_EXTNAME) && !defined(__ASSEMBLER__)";
    let Some(offset) = header.find(INSERT_BEFORE) else {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "BoringSSL prefix_symbols.h insertion point not found",
        ));
    };
    header.insert_str(offset, &format!("{definitions}\n"));
...
fn build_boringssl_or_get_prebuilt(config: &Config) -> &Path {
    ...
    BUILD_SOURCE_PATH.get_or_init(|| {
        ...
        if config.features.prefix_symbols {
            // BoringSSL's generated prefix headers and platform assembly cover the
            // complete public and internal symbol sets. PrefixCallback uses the
            // same crate-name prefix for Rust FFI declarations below.
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    prefix_patched_public_symbols(&config)?;
