...
impl Deflate {
    /// Bounds each decompressed message before growing the output buffer.
    ///
    /// The default is unbounded; callers should use their WebSocket message limit.
    pub fn set_max_message_size(&mut self, limit: usize) {
        self.decoder.max_message_size = limit;
    }
...
impl ExtensionEncoder for DeflateEncoder {
    ...
    fn encode(
        ...
        // Flush markers between fragments remain part of the same DEFLATE stream.
        // RFC 7692 removes only the final message trailer.
        if header.fin {
            buf.truncate(buf.len() - DEFLATE_TRAILER.len());
        }
        std::mem::swap(payload, buf);

        if header.fin && *compress_reset {
            compress.reset();
        }
...
impl ExtensionDecoder for DeflateDecoder {
    ...
    fn decode(
        ...
        loop {
            ...
            let status = decompress.decompress(&payload[offset..], &mut output, FlushDecompress::Sync)?;
            let consumed = (decompress.total_in() - before_in) as usize;
            let produced = (decompress.total_out() - before_out) as usize;
            offset += consumed;
            if produced > max_message_size.saturating_sub(buf.len()) {
                return Err(DeflateExtensionError::MessageTooLarge(*max_message_size));
            }
            buf.extend_from_slice(&output[..produced]);
            if status == Status::StreamEnd || (offset == payload.len() && produced < output.len()) {
                break;
            }
            if consumed == 0 && produced == 0 {
                return Err(DeflateExtensionError::InvalidCompressedMessage);
            }
        }
