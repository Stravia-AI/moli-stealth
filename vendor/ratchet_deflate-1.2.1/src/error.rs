...
pub enum DeflateExtensionError {
    /// The decompressed message exceeds the caller's limit.
    #[error("Decompressed WebSocket message exceeds {0} bytes")]
    MessageTooLarge(usize),
    /// A compressed payload cannot make progress while input remains.
    #[error("Invalid compressed WebSocket message")]
