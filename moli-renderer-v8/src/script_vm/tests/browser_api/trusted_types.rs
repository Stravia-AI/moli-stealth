...
#[test]
fn new_javascript_popup_uses_source_trusted_types_only_as_a_creation_preflight() {
    let mut blocked = new_storage_test_vm("https://source-trusted-types-popup-blocked.test/");
    blocked
        .set_response_content_security_policies(&["require-trusted-types-for 'script'".to_owned()]);
    let blocked_result = blocked
        .eval(
            r#"
(() => {
  globalThis.__sourcePopupTrustedTypesViolations = [];
  document.addEventListener("securitypolicyviolation", event => {
    __sourcePopupTrustedTypesViolations.push([
      event.disposition,
  ...
  try {
    opened = window.open(
      "javascript:globalThis.__blockedNewPopupRan = true",
      "source-trusted-types-blocked"
    );
  } catch (error) {
    exception = error.name;
  }
  return [opened === null, exception].join("|");
})()
"#,
        )
        .expect("blocked source Trusted Types popup probe should evaluate");
    assert_eq!(blocked_result, "true|none");
    assert!(blocked.take_pending_popup_activations().is_empty());
    assert_eq!(
    ...
    assert_eq!(
        blocked
            .eval("JSON.stringify(__sourcePopupTrustedTypesViolations)")
            .expect("source popup Trusted Types violation should be observable"),
        r#"["enforce|require-trusted-types-for|Location href|globalThis.__blockedNewPopupRan = true"]"#
    );

    let mut allowed = new_storage_test_vm("https://source-trusted-types-popup-allowed.test/");
    allowed
        .set_response_content_security_policies(&["require-trusted-types-for 'script'".to_owned()]);
...
(() => {
  ...
  const opened = window.open(
    "javascript:globalThis.__newPopupTargetOwnPolicyDecides = true",
    "source-trusted-types-allowed"
  );
  return JSON.stringify({ opened: opened !== null, calls });
})()
"#,
        )
        .expect("allowed source Trusted Types popup probe should evaluate");
    assert_eq!(
        allowed_result,
        r#"{"opened":true,"calls":[["globalThis.__newPopupTargetOwnPolicyDecides = true","TrustedScript","Location href"]]}"#
    );
    assert_eq!(allowed.take_pending_popup_activations().len(), 1);
    assert_eq!(
        drain_pre_domcontentloaded_non_script_page_tasks_for_test(&mut allowed),
        0
    );
}
