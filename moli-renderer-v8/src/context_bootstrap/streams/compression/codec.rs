...
mod tests {
    ...
    #[test]
    fn gzip_validates_header_crc() {
        let mut encoded = encode(Format::Gzip, b"header crc");
        encoded[3] |= 0x02;
        let mut crc = flate2::Crc::new();
        crc.update(&encoded[..10]);
        encoded.splice(10..10, (crc.sum() as u16).to_le_bytes());

        let (chunks, status) = Codec::new(Format::Gzip, true).process(&encoded, true);
        status.unwrap();
        assert_eq!(chunks.concat(), b"header crc");

        encoded[10] ^= 1;
        assert!(
            Codec::new(Format::Gzip, true)
                .process(&encoded, true)
                .1
                .is_err()
        );
    }
    ...
    #[test]
    fn zlib_rejects_dictionary_streams() {
        let dictionary_stream = [0x78, 0x20, 0, 0, 0, 0];
        assert!(
            Codec::new(Format::Deflate, true)
